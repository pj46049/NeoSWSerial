...
//
//
//  2020-02-02  Peter Jaeger    Extended to Arduino Nano Every in Emulation mode
//=============================================================================
...
static const uint8_t BITS_PER_TICK_38400_Q10 = 157;
    ...
    // PJ: On 4809, Timer 3B is used for Millis, TCA0 as pre-divider
    #define TCNTX TCB3.CNT
//    #define PCI_FLAG_REGISTER PCIFR // PJ: Not possible This concept does not exist on 4809!
  #else
    #define TCNTX TCNT0
...
void NeoSWSerial::listen()
{
  ...
  if (F_CPU == 8000000L) {
    ...
    #else // F_CPU e.g. 16000000L 
      #if defined(AVR_NANO_4809_328MODE)
        // PJ: Nano every uses 16MHz, so no action necessary. Millis() timer is used
      #else
        TCCR2A = 0x00;
...
#if defined(AVR_NANO_4809_328MODE)
  // PJ: Some identification of PinChangeInterrupt register here
  uint8_t bit_mask = digitalPinToBitMask(rxPin);
  if (bit_mask != NOT_A_PIN) { // PJ: Used different function
#ifdef PJDEBUG
  Serial.print("listen(): bit_mask=");
  ...
  if (pcmsk) {
    ...
    if(bit_mask != NOT_A_PIN)
        {
		// PJ: Enable pin change interrupts here!
		attachInterrupt(digitalPinToInterrupt(rxPin), pcint_isr, CHANGE);
		listener = this;
        }
    }
#else
...
void NeoSWSerial::ignore()
{
  if (listener) {
#if defined(AVR_NANO_4809_328MODE)
    // PJ: Find replacement for digitalPinToPCMSK(rxPin)
    /* Get pointer to correct pin control register */
    PORT_t *port = digitalPinToPortStruct(rxPin);
    uint8_t bit_pos = digitalPinToBitPosition(rxPin);
...
#if defined(AVR_NANO_4809_328MODE)
      // PJ: check for valid pin ID
      if(bit_pos != NOT_A_PIN)
        {
        /* PJ: Set interrupt on any change, enable Pullup to avoid ints on open pins */
        volatile uint8_t* pin_ctrl_reg = getPINnCTRLregister(port, bit_pos);
#ifdef PJDEBUG
  Serial.print("ignore(): pin_ctrl_reg=");
  Serial.print((uint32_t)pin_ctrl_reg, HEX);
#endif
...
void NeoSWSerial::nAttachInterrupt( isr_t fn )
{
...
void NeoSWSerial::rxISR( uint8_t port )
{
  uint8_t t0 = TCNTX;            // time of data transition (plus ISR latency)

#if defined(AVR_NANO_4809_328MODE)
  PORT_t *portStruct = portToPortStruct(port);
  portStruct->INTFLAGS = rxBitMask; // Clear flags that have been handled

  uint8_t d  = *portInputRegister(port) & rxBitMask; // read RX data level
#else
  uint8_t d  = port & rxBitMask; // read RX data level
#endif
...
//----------------------------------------------------------------------------
  ...
  #elif defined(AVR_NANO_4809_328MODE)
  // PJ: Pin Change Interrupt routine taken from WInterrupts.c

  void  pcint_isr()
    {
	uint8_t port = digitalPinToPort(listener->rxPin);
	if(port != NOT_A_PIN)
...
#if defined(AVR_NANO_4809_328MODE)
  // PJ: find port and bit position
  PORT_t *port = digitalPinToPortStruct(rxPin);
  uint8_t bit_mask = digitalPinToBitMask(rxPin);    
#else
    // PJ: Function not possible, different concept on 4809
...
#if defined(AVR_NANO_4809_328MODE)
        // PJ: Handle Receiver Interrupt here! 
        if(port->INTFLAGS & bit_mask){
          port->INTFLAGS |= bit_mask;
